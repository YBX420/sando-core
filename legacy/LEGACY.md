...
- **没折的**:`python/media/`(23M 演示 gif/图)、`_speed_bench.log`、`__pycache__`、以及 `cpp/`+`docs/`(被 sando-core 取代)。
- **完整副本去哪找**:① GitHub `YBX420/sando-py`——2026-06-22 已 **archive(只读)**,需要时 `gh repo unarchive YBX420/sando-py` 恢复;② 本地完整目录(含 .git + 所有分支)已移到 `../_retired_2026-06/sando-py/`(非破坏,可还原)。
...
- **现在去哪找**:sando-core 历史 `e8350cc`;GitHub `YBX420/sando-core` 分支 `bcert-wire`;本地断 worktree 目录已移到 `../_retired_2026-06/sando-core-bcert/`(非破坏)。
