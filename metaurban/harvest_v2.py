...
elif args.mode == "foldB6":
    jobs = [(n, CFG["SEEDS_FOLDB6"][n]) for n in POOL]
elif args.mode == "test6":
    jobs = [(n, s) for n in POOL for s in CFG["SEEDS_TEST6"][n]]
elif args.mode == "foldB7":
...
data = np.array(rows_all, dtype=[("d", "f4"), ("e", "f4"), ("age", "i4"), ("cls", "U12"),
                                 ...
                                 ("psig", "f4"), ("coast_s", "f4"),            # 07-20 contract: filter pos-sigma +
                                 ("a_held", "f4"), ("mem_m", "f4")])           # coast seconds + held |a| + applied mem margin
