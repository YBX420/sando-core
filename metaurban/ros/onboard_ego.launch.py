...
def generate_launch_description():
    ...
    def launch_setup(context, *args, **kwargs):
        ...
        # >>> SANDO swap (the ONLY functional change vs onboard_mighty.launch.py):
        #     our EGO planner instead of mighty_node; fake_sim/rsp/spawn/etc. identical. <<<
        ego_planner_proc = ExecuteProcess(
            cmd=['/usr/bin/python3',
                 os.path.join(os.path.dirname(os.path.abspath(__file__)), 'ego_node.py'),
                 '--ros-args', '-p', f'cruise_z:={float(z)}'],
            additional_env={'EGO_NS': namespace}, output='screen')
        nodes_to_start = [ego_planner_proc]
        if use_hardware:
