"""Isaac Sim (Ubuntu) <-> C++ SANDO 核心闭环。

只是 Isaac 的「方向盘」: 大脑(全局 HGP heat-A* + 局部 per-class MINCO + 证书)全在 C++ 核心
(cpp/capi/sando_capi.so),本脚本经 sando_cpp_bridge(ctypes)调它,完全照 sando_node 的节奏:
  每拍: update_state(当前位姿) -> add_traj(per-class DynTraj 障碍) -> replan()
        replan 内部跑 C++ 真管线: 全局 heat-A* 绕障 -> 局部 per-class MINCO -> append_to_plan
        get_next_goal() 从 plan 队列消费 setpoint 驱动无人机。
所有参数在同目录 isaac_sando.yaml 里,改那个调参,本脚本不用动。

引擎默认 = C++(SANDO_ENGINE=cpp)。早先 python 版警告的两处 bug(bbox 半/全尺寸约定、
v_max/a_max 没透传进 MINCO)在 C++ 核心已修: voxel_map.hpp 全程当全尺寸内部减半(factor-2 fix),
planner.hpp 把 par.v_max/a_max 透传进 opt.vmax/amax。故此处不再有那两处不一致。

跑法 (Ubuntu, 装了 Isaac Sim 的机器):
  # 用 Isaac 自带 python(其环境里要能 import numpy/yaml):
  ~/isaacsim/python.sh  /绝对路径/sando-core/isaac/isaac_loop.py
本机无 Isaac/无显示时,用 isaac/_smoke_headless.py 验证 C++ 闭环接线(不依赖 isaacsim 包)。
"""
import os
import sys
import time
...
_HERE = os.path.dirname(os.path.abspath(__file__))   # isaac/ —— 桥与 yaml 同目录
...
# ---- 落盘 ----
csv_path = os.path.join(_HERE, "isaac_flown.csv")
...
try:
    ...
    fig.savefig(os.path.join(_HERE, "isaac_flown.png"), dpi=120, bbox_inches="tight")
