...
## 视力手术 + 新面重锚(当日深夜,58e94c0..,塔菲大人:"drone 的 bbox 没有这么短,尽头内都能识别")
**--mover_range 30(认人)与 --fov_range 8(稠密深度物理)解耦**;双臂同视力(A/B 公平),真实前端距离噪声照涨;旧近视面 --mover_range 8 一键复现。**face-change 事件:渲染面全部锚点重立**(池/考卷账本 POOL_TAG 版本化,旧账保留)。
- **视力税全池坐实**:基线 hold 173→62(−64%),seed7 10.1/0.62→8.8/1.42,jerk −22%——两周里抢的指标有一大块是近视税;
- **σ余量(ETA_FEED=2 K=1.0)新面全面翻身**:全池 20/20 零碰、t 8.79(反超基线 −0.8s,11胜7负)、行人净空 1.60;**s5 口袋病随视力自愈**(病灶=近视×σ圈挤走廊合谋);唯一残留 s19 +20 hold 单例。**转正门槛全过,默认翻转待塔菲大人拍板**;
- **新面 n3 地狱卷(重新掐点 vcru 6.25)**:k10 9.8s/净空1.49/**零 hold** vs 基线 10.1/0.93/17 hold(近视面同卷 12.2/0.44/29)——成片二连已交;
- 新病靶更清晰:看得远+每拍重决=更早犹豫(近视 n3 spchurn 82.9),**M3 承诺机制的靶心**;
- 方法论:"一坨屎"审美判断→三方解剖(考卷税/感知税/架构税)→一行参数把整本账重写——**建模真实性 > 算法微调**。

## 新面全知抽查 + 账面勘误(07-17,塔菲大人令"测试全知/KF 场景")
- **全知零伤新面补全**:orabase/oraeta 新面 5/5 逐字节相同(s3/7/11/15/19,均时 8.38s、零 hold、零碰)——σ余量在全知臂硬置零如设计,K=1.0 转正证据链闭合;
- **账面勘误**:0c29c24 信里 base t9.61/hold62 系旧解析小误差;逐日志核实的正史=**base 9.73s/ped1.28/hold66,k10 8.91s/ped1.59/hold68**(差距 −0.82s、行人 +24%,方向结论不变);解析器已存盘 scratchpad/pool_parse.py(hold 可选、inf 兜底都焊死)。
