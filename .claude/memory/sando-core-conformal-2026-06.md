...
8. **render-faithful headless + 真 PX4(用户 6/24"headless 好渲染差,为什么 + 给 headless 也加上 + 我们有 PX4"):** 根因=真飞行动力学(headless 点质量完美跟踪;渲染/PX4 飞真四旋翼,实飞滞后认证的计划→蹭)+ 感知更脏 + 渲染路径旧未标定 keep-out(已修 v_eff 0.2→0.61)。修法:`replay_core --dynamics` 把 set-point 过**和渲染器同一个 `Quadrotor` 模型**(PX4 stand-in),净空测实飞位置。**全量 dynamics A/B:ours 0 碰撞、中位 −0.60s、min 净空 0.49m(裕度变薄→应标定跟踪误差 MAN_TRACK)。真 PX4 SITL spot-check(`px4_replay.py` + `flier` 钩子,MAVSDK offboard;PX4 SITL 被 Data21→Data2 CMake 缓存挡→直接起预编译 px4 二进制+jmavsim 绕过):seed1×3 ep ours 在真 PX4 上 0 碰撞、跟踪误差仅 0.17m;净空 PX4 2.3m > stand-in 1.0m > point-mass 1.5m → stand-in 偏保守不虚高。** caveat:PX4 这 3 ep 触发 early-stop(real-time position-offboard harness 交互,未到达),PX4 速度数字还不干净(安全已确认)。**坑:mavsdk 只装在 metaurban env;px4_replay 要 metaurban env + LD_PRELOAD sando 的 libstdc++(ego_capi+mavsdk 共存);PX4 build cache 是 Data21→Data2 老账(见 [[dev-env-data2-data21]])。** 启 PX4:`cd PX4-Autopilot/build/px4_sitl_default && HEADLESS=1 PX4_SIM_MODEL=jmavsim_iris setsid ./bin/px4 -d -s etc/init.d-posix/rcS` + jmavsim_run.sh。
