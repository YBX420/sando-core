...
5. **A/B 真轨迹回放** `replay_core.py`+`ab_replay.py`:真 mover 当**不让路录像**(strict),无人机飞穿走廊,ours=KF(CV)+per-class conformal 证书门控机动 vs native=贴 0.3 莽撞 EGO。**120 episode:ours 0 碰撞 / native 19;净空 ours 中位 1.48m / native 0.50m(撞到 −0.37);用时差中位 +0.00s、均值 −0.51s、59/118 严格更快、全部 ≤native+3s。验收 0 碰撞 PASS + within+3s PASS。** 仅 2 个 episode ours 慢 >2s(native 合法没撞)=认证安全在简单场景的诚实代价。
