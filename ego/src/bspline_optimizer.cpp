  ...
  bool BsplineOptimizer::rebound_optimize()
  {
    ...
    do
    {
      ...
      if (result == lbfgs::LBFGS_CONVERGENCE ||
          ...
          // line-search can't improve a near-optimal guess (e.g. empty FOV -> straight init already minimal).
          // The current trajectory is still valid; accept it if collision-free (matches CONVERGENCE handling).
          result == lbfgs::LBFGSERR_ROUNDING_ERROR ||
          result == lbfgs::LBFGSERR_MINIMUMSTEP ||
          result == lbfgs::LBFGSERR_MAXIMUMLINESEARCH ||
          result == lbfgs::LBFGSERR_WIDTHTOOSMALL ||
          result == lbfgs::LBFGSERR_INCREASEGRADIENT)
      {
