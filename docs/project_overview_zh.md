...
```
① 看:模拟相机给出检测(有遮挡、漏检、噪声)
② 追:每个人一个卡尔曼滤波器,估位置和速度,往后外推一秒
③ 加余量:预测会错。给每个人画一个"危险圈",圈的半径 = 身体 + 无人机 + 预测误差余量。
   预测越远,余量越大,圈越大
④ 规划:把"人将要在的位置"当障碍喂给规划器,它就会提前绕
⑤ 证明:对规划出的轨迹,数学证明"整段时间都在所有危险圈外面"
⑥ 执行:证过就飞。证不过,在几十个备选动作里挑一个能证过的。全灭,兜底逃
```
