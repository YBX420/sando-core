...
**逃生树:让兜底也尽量有证明(新,2026-07-13,`V2_ESC`,默认关)**。
道理先讲透:出事那拍再想办法已经晚了——那时无人机往往已经陷进膨胀的危险圈,
任何"停下来"的轨迹都证不过。退路必须**上一拍**就证好。
所以每拍提交轨迹时,顺手从"这拍飞完我会在哪"的预测状态,预证一条下一拍用的应急分支
(障碍也往前推一拍;观测新鲜度老老实实按"又旧了一拍"算,不偷)。
真到全灭那拍:先试现场造一条新鲜的"闪身刹停"(L1.5),造不出就飞上一拍的存货(L2)
——上一拍的证明窗本来就盖住这一拍,飞它仍在证明范围内;存货也证不过才裸逃,绝不冒充。
8 场景试跑:兜底 38→37,最难的 gauntlet 场景快 2.1 秒,**14 拍从"无证逃跑"换成"有证刹停"**,
零碰撞、无一场景变差(台账数字)。行为上收益不大——剩下的兜底集中在"深口袋",
预证的停和闪同样证不过,那是物理问题不是记账问题;真正的收益在第 10 节的总保证:
这 14 拍从"无保证"那一支挪进了"有证明"那一支。345 集全量判决还没跑。
...
### 13.1 逃生树:第一刀修法已落地(2026-07-13,`V3_ESC`,默认关)

上面那笔净空税的根源,是把"随时安全"定义成了**每条候选都要自己能原地刹停**。
逃生树(代码里叫预认证应急树)把它弱化成:**存在一条已经证明过的逃生路,就行**。

- **给被否的候选开树**:直刹证不过的候选不再直接扔——按"往目标的进度"挑前 12 名,
  每条开一张小逃生网格:朝背离威胁的方向(0°、±45°、±90°)闪开 1.5 或 3 米再停稳,
  先证前 0.3 秒,再证整段闪身(3 米这一档是打迎面场景的关键:
  闪身用的时间和危险圈生长速度在赛跑)。网格里证过一条,这个候选就算握着退路,准飞。
- **存货应急**:上一拍证好的逃生分支存着。全灭那拍先试现场新造(L1.5),
  造不出就飞存货(L2)——机制和 8.3 的 V2_ESC 是同一套。

**战果(回放面 8 场景,台账数字)**:兜底 104→46(−56%),crossers 场景兜底 27→0
还快 12.6 秒,零碰撞。合成的迎面悬崖测试里,3 米距离上被税掉的 14 条快速前进候选全部救回;
2 米以内从静止起步仍然全灭——但旧决策层在那里同样全灭,
那是预测管模型的物理地板,不是树的锅。

**和现役 V11 的正面对照(同池同旋钮)**:兜底 38 对 38 精确打平,净空还系统性更肥
(3.6–4.2 米对 1.9–3.2 米),**但到达只有 3/8 对 7/8**。这个对照把 v3 的病分成了两种:
递归可行性的净空税(逃生树治好了)≠ 慢爬/振荡不进展病(还没治,是下一刀)。
所以**现役仍是 V11 不换**;逃生树的机制先回灌给了现役决策层(见 8.3 的 V2_ESC)。
...
候补(默认关,未入现役):`V2_ESC`(逃生树接 V11,§8.3;记账升级,345 集全量判决未跑)、
`V3_ESC` 族(CPL-v3 专用逃生树,§13.1;方向/距离/预算旋钮 `V3_ESC_ANG/HOP/TOPK`)。
...
| 随时握着已证退路 | 递归可行性 |
| 逃生树 / 预认证应急树 | escape tree(`V3_ESC` / `V2_ESC`;提交拍预证=`precertify_branch`) |
| 现场新造逃生 / 飞存货 | L1.5 / L2(应急分层) |
